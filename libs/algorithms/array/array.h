...
/*возвращает индекс первого вхождения элемента x в массиве a размера n при наличии, иначе n*/
size_t linearSearch_(const int a[], const size_t n, int x);

/*возвращает индекс первого вхождения элемента x в отсортированном массиве а размера n при наличии, иначе n*/
size_t binarySearch_(const int a[], const size_t n, int x);

/*возвращает индекс первого элемента равного или большего x в отсортированном массиве a размера n
при отсутствии возврощает n*/
size_t binarySearchMoreOrEqual_(const int a[], const size_t n, int x);

/*вставка элемента со значением value в массив а размера n на позицию pos*/
void insert_(int a[], size_t *n, size_t pos, int value);

/*вставка элемента со значением value в конец массива a размера n*/
...
/*возвращает истина, если все элементы массива а размера n удовлетворяют функции-предикату predicate иначе ложь*/
int all_(const int a[], size_t n, int (*predicate)(int));

/*возвращает истина, если один элемент массива а размера n удовлетворяют функции-предикату predicate иначе ложь*/
int any_(const int a[], size_t n, int (*predicate)(int));

/*применяет фукнцию-предикат predicate ко всем элементам массива source сохраняет результат в массив dest размера n*/
...
/*удаляет из массива a размера n все элементы, соответсвующие функции-предикату deletePredicate, записывает в n
